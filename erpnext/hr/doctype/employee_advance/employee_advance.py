...
class EmployeeAdvance(Document):
    ...
    def validate(self):
        if self.refund_amount != 0 and self.return_journal is not None:
            frappe.throw("Please indicate the journal entry for the return")

        self.set_status()

    def on_change(self):
        if self.return_journal is not None:
            doc = frappe.get_doc("Journal Entry", self.return_journal)
            frappe.db.sql("""UPDATE `tabEmployee Advance` SET refund_amount = %s WHERE name = '%s' """ %
                          (self.refund_amount, self.name))
